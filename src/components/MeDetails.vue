...
<v-container class="elevation-0 mx-10 pa-0  rounded">
    ...
    <v-layout class="d-flex flex-wrap">
        <v-card class="w-100">
            <v-tabs v-model="tab">
                <v-tab value="me" to="/me" :class="tab==='me'? 'font-weight-bold':''" color="info" data-test-id="meDetails-meTab" prepend-icon="mdi-account">{{$t('meDetails.tabs.meLabel')}}</v-tab>
                <v-tab value="changePassword" :class="tab==='changePassword'? 'font-weight-bold':''"  to="/change-password" data-test-id="meDetails-changePasswordTab" color="info" prepend-icon="mdi-lock">{{$t('meDetails.tabs.changePasswordLabel')}}</v-tab>
                <v-tab value="changeEmail" :class="tab==='changeEmail'? 'font-weight-bold':''"  to="/change-email" data-test-id="meDetails-changeEmailTab" color="info" prepend-icon="mdi-at">{{$t('meDetails.tabs.changeEmailLabel')}}</v-tab>
                <v-tab value="settings" :class="tab==='settings'? 'font-weight-bold':''" to="/settings" data-test-id="meDetails-settingsTab" color="info" prepend-icon="mdi-cog">{{$t('meDetails.tabs.settingsLabel')}}</v-tab>
            </v-tabs>
